...
def improved2(query, inverted_index, freq, title_list):
    '''
    input query: 
    	inverted_index: is a dictioary, with keys as words and value as the posting list, so inverted_index['the'][0] = [0,756] . inverted_index['the'][0][1] is no of 'the' in 0th document
    	freq: it is a list of ditionaries, each dictionary is each doc's freq, freq[1]['the'] = 263 
...
def main():
    ...
    while 1:
        ...
        # this file stores the dictionary(like invertd index) but instead of posting list as keys, its keys are list of similar words
        ...
        if option=='1' :
            ...
            x = input("Which one you wanna go with, \n\t1.fast?(only some queries will run): Enter 1, or \n\t2.slow? Enter 2\n")
            if(x=='2') : 
